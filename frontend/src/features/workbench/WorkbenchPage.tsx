...
import { createWorkspace, deleteWorkspace, getWorkspaces } from "@/lib/api/workspaces";
import { ExecutionProcessesProvider } from "@/providers/ExecutionProcessesProvider";
import {
  useExecutionProcessesContext,
  isBusTaskStatusEvent,
  isBusTaskCreatedEvent,
  isBusIssueMergedEvent,
  isBusIssueAbandonedEvent,
...
import { cn, safeJsonNumberRecord } from "@/lib/utils";
import { createIssueAndInitialTask } from "@/features/workbench/workbenchActions";
import { readQaReportStatus, type QaReportStatus } from "@/features/workbench/qaReportStatus";
...
function WorkbenchInner({
  ...
  const orderedProjects = useMemo(() => {
    ...
    try {
      mru = safeJsonNumberRecord(window.localStorage.getItem("projectLastUsedAt") || "{}") ?? {};
    } catch {
      mru = {};
    }
  ...
  }, [addToast, currentTaskId, currentWorkspaceId, loadWorkspaceData]);
  ...
  const currentTasks = useMemo(
    () => (currentIssueId ? tasks.filter((t) => t.issue_id === currentIssueId) : []),
    [currentIssueId, tasks],
  );
  const hasArchitectureArtifacts = artifacts.some(
    (artifact) =>
      artifact.name === "architect/system_design.json" ||
      artifact.name === "architect/implementation_plan.json",
  );
  ...
  const allEngineerTasksDone =
    engineerTasks.length > 0 && engineerTasks.every((t) => t.status === "done");

  // qa_plan.json actually stores the QAReportDocument used for the status badge.
  const qaReportStatus = useMemo<QaReportStatus | null>(
    () => readQaReportStatus(artifacts),
    [artifacts],
  );
  ...
  const currentIssueCompletedProcessKey = useMemo(() => {
    ...
    return executionProcesses
      ...
      .map(
        (process) =>
          `${process.id}:${process.status}:${process.updated_at || process.completed_at || ""}`,
      )
      .sort()
      .join("|");
  ...
  const displayedProcessMessages = useMemo(() => {
    ...
    const liveProcessMessages = selectedProcess?.messages
      ? Object.values(selectedProcess.messages)
      : [];
  ...
  }, [processMessages, selectedProcess]);
  ...
  useEffect(() => {
    ...
    if (isBusTaskStatusEvent(lastEvent)) {
      ...
      setTasks((prev) =>
        prev.map((t) =>
          t.id === event.task_id
            ? { ...t, status: event.status, review_comment: event.review_comment ?? null }
            : t,
        ),
      );
  ...
  async function handleCreateIssue(
    ...
    try {
      ...
      if (!initialTask || !executionProcess) {
        throw new Error("Initial task runner did not start");
      }
  ...
  const handleTransitionToArchitecture = useCallback(async () => {
    ...
    try {
      ...
      setIssues((prev) =>
        prev.map((issue) => (issue.id === result.issue.id ? result.issue : issue)),
      );
  ...
  async function handleResolveApproval(itemId: string, decision: string, feedback: string | null) {
    try {
  ...
  const handleSendMessage = useCallback(
    async (content: string, mode: RunMode = "auto") => {
      ...
      try {
        ...
        const titleMap: Record<RunMode, string> = {
          auto: "发送失败",
          chat: "对话失败",
          refine: "修订失败",
        };
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-background font-sans">
                  ...
                  <DropdownMenuItem
                    ...
                    onClick={() => {
                      ...
                      const hasRunning = tasks.some(
                        (tk) => tk.status === "running" || tk.status === "responding",
                      );
                        ...
                        try {
                          const mru =
                            safeJsonNumberRecord(
                              window.localStorage.getItem("projectLastUsedAt") || "{}",
                            ) ?? {};
                          mru[p.id] = Date.now();
      ...
      <Sheet open={!!currentTaskId} onOpenChange={(open) => !open && setCurrentTaskId(null)}>
        <SheetContent
          side="right"
          className="w-[90vw] sm:max-w-[1400px] h-full p-0 flex flex-col gap-0 border-l border-border-subtle bg-surface shadow-2xl overflow-hidden"
        >
                ...
                <div className="p-8 border-b border-border-subtle bg-surface/30">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="px-3 py-1 rounded-lg bg-surface-raised border border-border-subtle text-[10px] font-black uppercase tracking-widest text-text-secondary">
                      {t("task.role")}:{" "}
                      {tasks.find((t) => t.id === currentTaskId)?.role.replace("_", " ")}
                    </div>
                    <div className="px-3 py-1 rounded-lg bg-surface-raised border border-border-subtle text-[10px] font-black uppercase tracking-widest text-text-secondary">
                  ...
                  <div className="flex-1 min-h-0">
                    <TabsContent value="run-detail" className="h-full m-0 flex flex-col">
                      <RunDetail
                        ...
                        isLoadingMessages={
                          isLoadingMessages && displayedProcessMessages.length === 0
                        }
                        ...
                        onRunAgain={async (executor, provider, model) => {
                          if (selectedProcess) {
                            try {
                              const result = await rerunCodexTask(selectedProcess.task_id, {
                                executor,
                                provider,
                                model,
                              });
                        ...
                        onTerminate={async () => {
                          if (currentTaskId) {
                            try {
                              ...
                              setError(
                                err instanceof Error ? err.message : "Failed to terminate task",
                              );
                            }
      ...
      <MacroRecorder open={macroRecorderOpen} onOpenChange={setMacroRecorderOpen} />
