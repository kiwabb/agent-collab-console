...
function getDevelopmentTaskUnlockStatus(
  task: CodexTask,
  allTasks: CodexTask[],
...
export function RunDetail({
  ...
  const baseExecutionConfig = useMemo<ExecutionConfigValue>(
    () => getFallbackConfig(catalog || null, selectedExecutor, selectedProvider, selectedModel),
  ...
  const canRunInitial =
    !process && !!taskMeta && (taskStatus === "pending" || taskStatus === "failed");
  const unlockStatus = useMemo(
    () =>
      taskMeta
        ? getDevelopmentTaskUnlockStatus(taskMeta, allTasks)
  ...
  return (
    <div className="flex flex-col h-full bg-background overflow-hidden">
      ...
      <div className="p-5 border-b border-border-subtle bg-surface/50">
            ...
            <>
              {showTransitionToArchitecture && taskMeta?.phase === "requirements" && (
                <button
                  ...
                  data-density={
                    isTransitioningToArchitecture
                      ? "run-detail-transition-architecture-dispatch"
                      : "run-detail-transition-architecture"
                  }
                  className={cn(
              ...
              {showTransitionToDevelopment && taskMeta?.phase === "architecture" && (
                <button
                  ...
                  data-density={
                    isTransitioningToDevelopment
                      ? "run-detail-transition-development-dispatch"
                      : "run-detail-transition-development"
                  }
                  className={cn(
                    "flex-2 inline-flex items-center justify-center gap-2 px-4 py-2 text-xs font-bold uppercase tracking-widest rounded-lg bg-brand text-background hover:bg-brand/90 transition-all shadow-md active:scale-[0.98] disabled:opacity-50",
              ...
              {showTransitionToTesting &&
                (taskMeta?.phase === "development" || taskMeta?.phase === "testing") && (
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <button
                        ...
                        data-density={
                          isTransitioningToTesting
                            ? "run-detail-transition-testing-dispatch"
                            : "run-detail-transition-testing"
                        }
                        className={cn(
                          "flex-2 inline-flex items-center justify-center gap-2 px-4 py-2 text-xs font-bold uppercase tracking-widest rounded-lg bg-brand text-background hover:bg-brand/90 transition-all shadow-md active:scale-[0.98] disabled:opacity-50",
              ...
              {taskMeta?.phase === "development" &&
                ...
                onSubmitForReview && (
                  <div className="flex-2 inline-flex items-center justify-center gap-2 px-4 py-2 text-xs font-bold uppercase tracking-widest rounded-lg bg-warning/10 text-warning border border-warning/30">
                    <AlertCircle size={14} />
                    <span>
                      {t("task.status.needDoneToSubmit").replace(
                        "{status}",
          ...
          <div className="mt-4 flex max-h-48 gap-3 rounded-xl border border-error/25 bg-error/10 px-4 py-3 text-error">
            ...
            <div className="min-w-0 overflow-y-auto pr-1">
              <p className="text-[10px] font-black uppercase tracking-[0.18em]">
                {errorEntry.label || t("run.error")}
              </p>
              <p className="mt-1 text-xs leading-relaxed break-words whitespace-pre-wrap text-error/90">
                {errorEntry.content}
              </p>
            </div>
          </div>
        ...
        {taskStatus === "failed" && taskMeta?.result && (
          <div className="mt-4 flex max-h-48 gap-3 rounded-xl border border-error/25 bg-error/10 px-4 py-3 text-error">
            ...
            <div className="min-w-0 overflow-y-auto pr-1">
              <p className="text-[10px] font-black uppercase tracking-[0.18em]">
                {t("task.executionFailed")}
              </p>
              <p className="mt-1 text-xs leading-relaxed break-words text-error/90 whitespace-pre-wrap">
                {taskMeta.result}
              </p>
            </div>
          </div>
      ...
      {(() => {
        const isQaFailed =
          taskMeta?.role === "qa" && taskMeta?.status === "failed" && taskMeta?.review_comment;
        const isArchitectReview =
          (taskMeta?.status === "rework" || taskMeta?.status === "done") &&
          taskMeta?.review_comment &&
          taskMeta?.role !== "qa";
            ...
            <div
              className={cn(
                ...
                isError
                  ...
                  : "bg-success/10 border border-success/20 shadow-lg shadow-success/5",
              )}
            >
              <h4
                className={cn(
        ...
        <Tabs defaultValue="messages" className="flex-1 flex flex-col min-h-0">
          <div className="px-5 border-b border-border-subtle bg-surface/20">
            <TabsList className="bg-transparent h-12 w-full justify-start gap-6 p-0">
              ...
              >
                {t("run.logs")}
                <span className="ml-2 text-[9px] opacity-40 font-mono">
          ...
          <div className="flex-1 min-h-0 relative">
            <TabsContent
              ...
              value="messages"
              className="absolute inset-0 m-0 overflow-y-auto p-6 space-y-6 animate-in fade-in duration-300"
            >
              {isLoadingMessages ? (
                <div
            ...
            <TabsContent
              ...
              value="logs"
              className="absolute inset-0 m-0 p-6 bg-background/40"
            >
      ...
      <div className="p-5 border-t border-border-subtle bg-surface/50 space-y-3">
        ...
        <div className="flex items-center gap-2">
          {[
            {
              id: "auto" as RunMode,
              label: t("task.runMode.auto"),
              hint: t("task.runMode.autoHint"),
            },
            {
              id: "chat" as RunMode,
              label: t("task.runMode.chat"),
              hint: t("task.runMode.chatHint"),
            },
            {
              id: "refine" as RunMode,
              label: t("task.runMode.refine"),
          ...
          {runMode === "auto" && lastResolvedMode && (
            ...
            >
              {lastResolvedMode === "refine"
                ? t("task.runMode.autoResult")
                : t("task.runMode.autoResultChat")}
            </span>
          )}
        </div>
        <form onSubmit={handleSend} className="flex gap-4">
          <div className="flex-1 relative group">
            <input
              ...
              placeholder={
                runMode === "refine" ? t("run.refinePlaceholder") : t("run.messagePlaceholder")
              }
            ...
            <div className="absolute right-5 top-1/2 -translate-y-1/2 opacity-0 group-focus-within:opacity-100 transition-all scale-90 group-focus-within:scale-100 pointer-events-none">
              <div className="px-2 py-1 rounded-md border border-border-strong text-[9px] font-black text-text-muted uppercase tracking-widest bg-surface-raised">
                Enter
              </div>
            </div>
          </div>
...
function QaReportBadge({
  ...
  const { t } = useI18n();
  const styles: Record<QaReportStatus, { cls: string; label: string }> = {
...
function StatusBadge({ status }: { status: string }) {
  ...
  const map: Record<string, string> = {
    ...
    awaiting_review:
      "bg-warning/10 text-warning border-warning/30 shadow-[0_0_8px_rgba(234,179,8,0.1)]",
