    ...
    <div className="mt-2">
      ...
      {open && (
        <div className="mt-2 space-y-2">
          {turns.map((turn) => (
            <pre key={turn.id} className="max-h-44 overflow-auto whitespace-pre-wrap rounded-xl border border-border-subtle bg-background p-3 text-[11px] leading-relaxed text-text-secondary">
              {readableTurn(turn)}
            </pre>
          ))}
        </div>
