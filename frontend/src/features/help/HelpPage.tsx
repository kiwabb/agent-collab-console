    ...
    >
      ...
      <section>
        ...
        <ul className="text-[13px] space-y-2 text-text-secondary leading-relaxed">
          <li>
            <b className="text-foreground">{t("help.concept.project")}</b> —{" "}
            <span dangerouslySetInnerHTML={{ __html: t("help.concept.project.body") }} />
          </li>
          <li>
            <b className="text-foreground">{t("help.concept.workspace")}</b> —{" "}
            <span dangerouslySetInnerHTML={{ __html: t("help.concept.workspace.body") }} />
          </li>
          <li>
            <b className="text-foreground">{t("help.concept.issue")}</b> —{" "}
            <span dangerouslySetInnerHTML={{ __html: t("help.concept.issue.body") }} />
          </li>
          <li>
            <b className="text-foreground">{t("help.concept.dag")}</b> —{" "}
            <span dangerouslySetInnerHTML={{ __html: t("help.concept.dag.body") }} />
          </li>
          <li>
            <b className="text-foreground">{t("help.concept.task")}</b> —{" "}
            <span dangerouslySetInnerHTML={{ __html: t("help.concept.task.body") }} />
          </li>
          <li>
            <b className="text-foreground">{t("help.concept.run")}</b> —{" "}
            <span dangerouslySetInnerHTML={{ __html: t("help.concept.run.body") }} />
          </li>
          <li>
            <b className="text-foreground">{t("help.concept.artifact")}</b> —{" "}
            <span dangerouslySetInnerHTML={{ __html: t("help.concept.artifact.body") }} />
          </li>
          <li>
            <b className="text-foreground">{t("help.concept.steer")}</b> —{" "}
            <span dangerouslySetInnerHTML={{ __html: t("help.concept.steer.body") }} />
          </li>
          <li>
            <b className="text-foreground">{t("help.concept.memory")}</b> —{" "}
            <span dangerouslySetInnerHTML={{ __html: t("help.concept.memory.body") }} />
          </li>
        </ul>
      </section>
    </PageFrame>
  ...
  return (
    <li className="enterprise-card rounded-2xl p-4 flex gap-4">
      ...
      <div className="min-w-0 flex-1">
        ...
        {cta && (
          <Button size="sm" variant="outline" onClick={cta.onClick} className="mt-2 text-[12px]">
            {cta.label} →
          </Button>
        )}
      </div>
...
function ShortcutRow({ keys, alt, desc }: { keys: string[]; alt?: string; desc: string }) {
  return (
    <div className="px-4 py-2.5 flex items-center gap-3">
      <div className="flex items-center gap-1 min-w-[120px]">
        ...
        {alt && <span className="text-[10px] text-text-muted ml-2">/ {alt}</span>}
      </div>
