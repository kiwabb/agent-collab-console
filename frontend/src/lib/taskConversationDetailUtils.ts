...
export type TaskConversationLog = Record<string, unknown> & {
  id: string;
  created_at?: string | null;
};

function isTaskConversationLog(value: unknown): value is TaskConversationLog {
...
export function sortMessages<T extends { created_at?: string | null; role?: string }>(
  messages: T[],
): T[] {
...
export function mergeTaskConversationMessages(
  messageGroups: CodexTaskMessage[][],
): CodexTaskMessage[] {
  return sortMessages(mergeById(messageGroups.flat()));
}

export function mergeTaskConversationLogs(logGroups: unknown[][]): TaskConversationLog[] {
  return sortLogs(mergeById(logGroups.flat().filter(isTaskConversationLog)));
}
  ...
  const syntheticMessages: CodexTaskMessage[] = normalizedLogs
    .filter(
      (entry: NormalizedEntry) =>
        entry.type === "assistant" && typeof entry.content === "string" && entry.content,
    )
    .filter(
      (entry: NormalizedEntry) =>
        !entry.executionProcessId || !persistedAssistantProcessIds.has(entry.executionProcessId),
    )
    .filter(
      (entry: NormalizedEntry) =>
        !persistedAssistantKeys.has(`${entry.executionProcessId || "none"}::${entry.content}`),
    )
