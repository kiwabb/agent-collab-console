...
test("diff merge create PR cta uses dispatch motion while opening GitHub PR", () => {
  ...
  assert.doesNotMatch(
    source,
    /busy === "pr-create" \? \(\s*<span className="flex items-center gap-1\.5">\s*<Loader2 size=\{12\} className="animate-spin"/,
  );
});
...
test("diff merge merge-back cta uses dispatch motion while merging", () => {
  ...
  assert.match(
    source,
    /busy === "merge" \? \(\s*<span className="flex items-center gap-1\.5">\s*<AgentThinkingIndicator phase="dispatching" size=\{12\}/,
  );
  assert.doesNotMatch(
    source,
    /busy === "merge" \? \(\s*<span className="flex items-center gap-1\.5">\s*<Loader2 size=\{12\} className="animate-spin"/,
  );
});
...
test("diff merge toolbar ctas use dispatch motion while busy", () => {
  ...
  assert.match(
    source,
    /busy === "pr-create" \? \(\s*<AgentThinkingIndicator phase="dispatching" size=\{11\}/,
  );
  assert.doesNotMatch(
    source,
    /busy === "pr-create" \? \(\s*<Loader2 size=\{11\} className="animate-spin" \/>/,
  );
  ...
  assert.match(
    source,
    /busy === "merge" \? \(\s*<span className="flex items-center gap-1\.5">\s*<AgentThinkingIndicator phase="dispatching" size=\{11\}/,
  );
  assert.doesNotMatch(
    source,
    /busy === "merge" \? \(\s*<span className="flex items-center gap-1\.5">\s*<Loader2 size=\{11\} className="animate-spin" \/>/,
  );
});
...
test("diff merge abandon cta uses dispatch motion while abandoning", () => {
  ...
  assert.match(
    source,
    /busy === "abandon" \? \(\s*<span className="flex items-center gap-1\.5">\s*<AgentThinkingIndicator phase="dispatching" size=\{12\}/,
  );
  assert.doesNotMatch(
    source,
    /busy === "abandon" \? \(\s*<span className="flex items-center gap-1\.5">\s*<Loader2 size=\{12\} className="animate-spin"/,
  );
});
...
test("diff merge PR refresh cta uses tool motion while syncing GitHub PR state", () => {
  ...
  assert.match(
    source,
    /busy === "pr-refresh" \? \(\s*<AgentThinkingIndicator phase="tool" size=\{12\}/,
  );
  assert.doesNotMatch(
    source,
    /busy === "pr-refresh" \? \(\s*<Loader2 size=\{12\} className="animate-spin"/,
  );
});
...
test("diff merge refresh cta uses tool motion while syncing diff state", () => {
  ...
  assert.match(
    source,
    /isRefreshing \? \(?\s*<AgentThinkingIndicator phase="tool" size=\{12\} \/>/,
  );
...
test("diff merge submit review cta uses thinking motion while submitting", () => {
  ...
  assert.match(
    source,
    /busy === "submit" \? \(\s*<span className="flex items-center gap-1\.5">\s*<AgentThinkingIndicator phase="thinking" size=\{12\}/,
  );
  assert.doesNotMatch(
    source,
    /busy === "submit" \? \(\s*<span className="flex items-center gap-1\.5">\s*<Loader2 size=\{12\} className="animate-spin"/,
  );
});
...
test("diff merge review decision ctas use thinking motion while approving or rejecting", () => {
  ...
  assert.match(
    source,
    /busy === "review-approve" \? \(\s*<span className="flex items-center gap-1\.5">\s*<AgentThinkingIndicator phase="thinking" size=\{12\}/,
  );
  assert.doesNotMatch(
    source,
    /busy === "review-approve" \? \(\s*<span className="flex items-center gap-1\.5">\s*<Loader2 size=\{12\} className="animate-spin"/,
  );
  assert.match(
    source,
  ...
  assert.match(
    source,
    /isLoading \? \(\s*<span className="flex items-center gap-1\.5">\s*<AgentThinkingIndicator phase="thinking" size=\{12\}/,
  );
  assert.doesNotMatch(
    source,
    /isLoading \? \(\s*<span className="flex items-center gap-1\.5">\s*<Loader2 size=\{12\} className="animate-spin"/,
  );
});
