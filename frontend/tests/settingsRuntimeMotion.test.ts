...
test("settings runtime catalog loading uses tool motion", () => {
  const source = readFileSync(join(SRC_ROOT, "features/settings/SettingsPage.tsx"), "utf8");
  ...
  assert.match(
    source,
    /animate-shimmer-sweep bg-gradient-to-r from-transparent via-status-tool\/70 to-transparent/,
  );
