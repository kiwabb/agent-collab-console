...
test("agent live execution controls use semantic busy motion", () => {
  ...
  assert.match(
    source,
    /stopBusy \? \(\s*<AgentThinkingIndicator phase="tool" size=\{10\} \/>\s*\) : \(\s*<Square size=\{10\} className="fill-error" \/>\s*\)/,
  );
  assert.doesNotMatch(
    source,
    /stopBusy \? \(\s*<Loader2 size=\{10\} className="animate-spin" \/>\s*\) :/,
  );
  ...
  assert.match(
    source,
    /rerunBusy \? \(\s*<AgentThinkingIndicator phase="dispatching" size=\{11\} \/>\s*\) : \(\s*<RotateCcw size=\{11\} \/>\s*\)/,
  );
  assert.doesNotMatch(
    source,
    /rerunBusy \? \(\s*<Loader2 size=\{11\} className="animate-spin" \/>\s*\) :/,
  );
});
