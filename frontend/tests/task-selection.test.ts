...
test("groupIssuesByPhase groups issues by current_phase", () => {
  ...
  assert.equal(result.requirements[0].id, "i1");
  assert.equal(result.architecture.length, 1);
  assert.equal(result.architecture[0].id, "i2");
  assert.equal(result.development.length, 1);
  assert.equal(result.development[0].id, "i3");
  assert.equal(result.testing.length, 1);
  assert.equal(result.testing[0].id, "i4");
});
...
test("deriveIssueCounts counts tasks and processes for an issue", () => {
  ...
  const helpRequests: HelpRequest[] = [
    { id: "h1", workspace_id: "w1", parent_task_id: "t1", child_task_id: "t2", source_executor: "codex", target_executor: "claude", title: "Help", prompt: "", context_summary: null, status: "pending", error_message: null, continuation_payload: null, created_at: null, started_at: null, completed_at: null, timeout_at: null, consumed_at: null },
  ];
...
test("pickDisplayedExecutionProcess returns selected process when currentProcessId is set", () => {
  const processes: ExecutionProcess[] = [
    { id: "p1", task_id: "t1", session_id: "s1", status: "completed", exit_code: null, started_at: null, completed_at: null, created_at: "2026-04-18T10:00:00Z", updated_at: null },
    { id: "p2", task_id: "t1", session_id: "s1", status: "running", exit_code: null, started_at: null, completed_at: null, created_at: "2026-04-18T10:05:00Z", updated_at: null },
  ];
...
test("pickLatestExecutionProcessForTask returns the newest process for a task", () => {
  const processes: ExecutionProcess[] = [
    { id: "p1", task_id: "t1", session_id: "s1", status: "completed", exit_code: null, started_at: null, completed_at: null, created_at: "2026-04-18T10:00:00Z", updated_at: null },
    { id: "p2", task_id: "t1", session_id: "s1", status: "running", exit_code: null, started_at: null, completed_at: null, created_at: "2026-04-18T10:05:00Z", updated_at: null },
    { id: "p3", task_id: "t2", session_id: "s1", status: "running", exit_code: null, started_at: null, completed_at: null, created_at: "2026-04-18T10:10:00Z", updated_at: null },
  ];
...
test("getTaskRuntimeStatus prefers the latest execution process over stale task status", () => {
  ...
  const processes: ExecutionProcess[] = [
    { id: "p1", task_id: "t1", session_id: "s1", status: "completed", exit_code: null, started_at: null, completed_at: null, created_at: "2026-04-18T10:05:00Z", updated_at: null },
  ];
...
test("isTaskRuntimeActive uses the latest execution process instead of stale task status", () => {
  ...
  const processes: ExecutionProcess[] = [
    { id: "p1", task_id: "t1", session_id: "s1", status: "Completed", exit_code: 0, started_at: null, completed_at: "2026-04-18T10:05:00Z", created_at: "2026-04-18T10:00:00Z", updated_at: "2026-04-18T10:05:00Z" },
  ];
