...
test("workspace grid loading uses dispatch motion", () => {
  const source = readFileSync(
    path.join(root, "src/features/workspaces/WorkspaceGrid.tsx"),
    "utf8",
  );
  ...
  assert.match(source, /className="motion-essential relative col-span-full flex min-h-\[220px\] items-center justify-center gap-2 overflow-hidden rounded-lg border border-status-running\/25 bg-status-running\/5 text-sm font-semibold text-text-muted"/);
