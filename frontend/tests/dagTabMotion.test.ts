...
test("dag tab running graph status uses dispatch motion", () => {
  ...
  assert.match(
    source,
    /data-density=\{\s*graph\.status === "running"\s*\? "dag-tab-running-status"\s*: "dag-tab-status"\s*\}/,
  );
  assert.match(source, /graph\.status === "running" && "motion-essential"/);
  assert.match(
    source,
    /graph\.status === "running" && \(\s*<AgentThinkingIndicator phase="dispatching" size=\{10\} \/>/,
  );
  assert.doesNotMatch(
    source,
...
test("dag tab start conductor ctas use dispatch motion while scheduling", () => {
  ...
  assert.match(
    source,
    /data-density=\{\s*busy\s*\? "dag-tab-start-conductor-dispatch-cta"\s*: "dag-tab-start-conductor-cta"\s*\}/,
  );
  assert.match(
    source,
    /data-density=\{\s*busy\s*\? "dag-tab-toolbar-start-dispatch-cta"\s*: "dag-tab-toolbar-start-cta"\s*\}/,
  );
  assert.match(
    source,
    /className=\{cn\(\s*"bg-brand text-black hover:bg-brand-strong font-semibold shadow-sm",\s*busy && "motion-essential",\s*\)\}/,
  );
  assert.match(
    source,
    /className=\{cn\(\s*"h-7 px-2\.5 text-\[12px\] bg-brand hover:bg-brand-strong text-black font-semibold",\s*busy && "motion-essential",\s*\)\}/,
  );
  assert.match(
    source,
  ...
  assert.match(
    source,
    /busy \? \(\s*<>\s*<AgentThinkingIndicator phase="dispatching" size=\{11\} \/>/,
  );
...
test("dag tab retry confirmation uses dispatch motion while redispatching", () => {
  ...
  assert.match(dialogSource, /loadingMotionPhase\?: string \| undefined;/);
  assert.match(dialogSource, /loadingDensity\?: string \| undefined;/);
  assert.match(dialogSource, /loadingIndicatorSize\?: number \| undefined;/);
  ...
  assert.match(
    dialogSource,
    /isLoading \? \(\s*loadingMotionPhase \? \(\s*<AgentThinkingIndicator\s*phase=\{loadingMotionPhase\}\s*size=\{loadingIndicatorSize\}/,
  );
});
