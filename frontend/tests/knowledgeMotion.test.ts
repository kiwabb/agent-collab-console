...
test("knowledge reindex cta uses tool motion while rebuilding memory", () => {
  ...
  assert.match(
    source,
    /className=\{cn\(\s*"inline-flex items-center gap-1 rounded-md border border-border bg-surface px-2 py-1 text-xs hover:bg-surface-hover disabled:opacity-50",\s*reindexing && "motion-essential",\s*\)\}/,
  );
  assert.match(source, /reindexing \? \(?\s*<AgentThinkingIndicator phase="tool" size=\{12\} \/>/);
