...
def test_diagnostics_returns_operational_snapshot(client):
    ...
    assert body["self_improvement_proposal_scheduler"]["configured"] is True
    assert body["self_improvement_proposal_scheduler"]["interval_s"] == 3600.0
    assert body["self_improvement_proposal_scheduler"]["limit"] == 25
...
def test_diagnostics_includes_self_improvement_proposal_scheduler_error(client):
    from app.application.github_pr_followup import reset_github_pr_followup_status
    from app.application.project_review_scheduler import reset_project_review_scheduler_status
    from app.application.self_improvement_proposal_scheduler import (
        reset_self_improvement_proposal_scheduler_status,
    ...
    async def tick(store, *, activate_fn, limit=None):
        raise RuntimeError("proposal store temporarily unavailable")
    ...
    assert resp.status_code == 200
    scheduler = resp.json()["self_improvement_proposal_scheduler"]
    assert scheduler["interval_s"] == 11
    assert scheduler["limit"] == 4
    assert scheduler["last_error"] == "RuntimeError: proposal store temporarily unavailable"
    body = resp.json()
    assert body["status"] == "degraded"
    assert any(
        check["name"] == "self_improvement_proposal_scheduler"
        and check["status"] == "degraded"
        and check["detail"] == "RuntimeError: proposal store temporarily unavailable"
        for check in body["checks"]
    )
...
def test_diagnostics_degrades_when_self_improvement_proposal_scheduler_is_running(client, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: {
            ...
            "running": True,
            "tick_count": 5,
            "last_started_at": "2026-06-09T10:00:00",
            "last_completed_at": None,
            "last_error": None,
            "last_summary_counts": {},
        },
    ...
    body = resp.json()
    assert body["status"] == "degraded"
    assert body["self_improvement_proposal_scheduler"]["running"] is True
    assert any(
        check["name"] == "self_improvement_proposal_scheduler"
        and check["status"] == "degraded"
        and check["detail"] == "Self-improvement proposal scheduler is running"
        for check in body["checks"]
    )
...
def test_diagnostics_degrades_when_self_improvement_proposal_scheduler_is_stale(client, monkeypatch):
    ...
    assert any(
        check["name"] == "self_improvement_proposal_scheduler"
        and check["status"] == "degraded"
        and check["detail"] == "Self-improvement proposal scheduler has not completed recently"
        for check in body["checks"]
    )
