...
async def test_initial_checkpoint_and_command_recover_as_one_durable_chain(
    ...
    try:
        appended = await store.append_command_batch(
            batch=batch,
        ...
        assert persisted_operation.result_manifest_hash == operation.result_manifest_hash
    finally:
        await store.close()
...
async def test_command_retry_returns_original_result_without_new_event(tmp_path: Path) -> None:
    ...
    try:
        first = await store.append_command_batch(
            batch=batch,
            completed_operation=operation,
            completion_step=step,
            completion_event=event,
        )
        second = await store.append_command_batch(
...
async def test_two_connections_cannot_append_the_same_draft_head(tmp_path: Path) -> None:
    ...
    try:
        ...
        persisted = await first_store.load_draft("draft-1")
        assert persisted is not None
...
async def test_replay_tail_limit_refuses_batch_before_the_201st_tail_entry(
    ...
    store, _, initial, _, _ = await _create_initial_draft(tmp_path)
    conn = await store._get_conn()
    await conn.execute(
        "UPDATE prototype_drafts SET head_sequence_no = ?, head_document_hash = ? WHERE id = ?",
        (MAX_REPLAY_TAIL_BATCHES, _hash("e"), "draft-1"),
    )
    await conn.commit()
    batch, operation, step, event = await _prepare_command(
        store,
...
async def test_runtime_event_append_is_optimistic_and_idempotent(tmp_path: Path) -> None:
    store, _, initial_state, session, _ = await _create_initial_runtime_session(tmp_path)
    now = datetime.now(UTC)
    ...
    try:
        first = await store.append_runtime_event_batch(
            event_batch=event_batch,
            completed_operation=operation,
            completion_step=step,
            completion_event=event,
        )
        second = await store.append_runtime_event_batch(
...
async def test_two_connections_cannot_append_the_same_runtime_head(tmp_path: Path) -> None:
    first_store, _, initial_state, _, _ = await _create_initial_runtime_session(tmp_path)
    second_store = AsyncStructuredPrototypeStore(tmp_path / "structured.db")
    now = datetime.now(UTC)
    ...
    async def append(
        ...
        event_batch, operation, step, event = prepared
        try:
            await store.append_runtime_event_batch(
                event_batch=event_batch,
                completed_operation=operation,
                completion_step=step,
                completion_event=event,
...
async def test_runtime_replay_tail_refuses_the_201st_event(tmp_path: Path) -> None:
    store, _, _, _, _ = await _create_initial_runtime_session(tmp_path)
    conn = await store._get_conn()
    await conn.execute(
        """
...
async def test_runtime_checkpoint_resets_the_replay_tail(tmp_path: Path) -> None:
    ...
    try:
        checkpointed = await store.register_runtime_checkpoint(
            descriptor=next_state,
            reference=reference,
            checkpoint=checkpoint,
...
async def test_runtime_recovery_failure_marks_the_same_session_head_corrupt(
    ...
    appended = await store.append_runtime_event_batch(
        event_batch=event_batch,
        completed_operation=operation,
        completion_step=step,
        completion_event=event,
...
@pytest.mark.asyncio
async def test_main_async_store_migrates_structured_schema_to_version_11(tmp_path: Path) -> None:
