        ...
        tasks=[
            _task(
                ...
                result_json={
                    ...
                    "commands": [
                        {"cmd": "pytest backend/tests/test_qa_workflow.py", "exit_code": 1}
                    ],
                },
            )
        ]
...
async def test_runtime_failure_creates_runtime_tooling_proposal():
    store = MemoryStore(
        tasks=[_task("task-1", result_json={"traceback": "RuntimeError: browser observed failure"})]
    )

    proposals = await extract_self_improvement_proposals(
        _issue(title="Fix browser runtime failure"), store
    )
...
async def test_capability_issue_without_eval_evidence_creates_benchmark_eval_proposal():
    ...
    store = MemoryStore(
        tasks=[
            _task(
                ...
                result_json={
                    "summary": (
                        "implemented capability improvement without measured acceptance evidence"
                    )
                },
            )
        ]
    ...
    assert (
        proposal.fingerprint == "project-1|issue-1|benchmark_eval|missing_capability_eval_contract"
    )
