...
EventBus event, agent finalize — funnels through a `AuditSink.record(...)` call
(the shaping lives in `audit.recorders`; this module is the adapter that
actually persists). Centralizing the write here is deliberate: one writer means
the instrumentation points can never drift into double-writes or inconsistent
schemas.
...
This module is the `AuditSink` adapter (hexagonal port: `app.domain.ports`).
Business modules depend only on the `AuditSink` Protocol + the typed recorder
...
# Bounded-queue backpressure. The high-frequency `event_bus.append` and
# `git_service._run` choke points feed the audit trail, so an unbounded queue
# could grow without limit if the DB drain falls behind (e.g. a slow disk during
# a burst of fan-out events) and eventually OOM the process. Audit logging is
# best-effort, so a bounded queue with a DROP policy is the correct trade:
# losing a few audit rows under extreme load is strictly better than crashing
# the thing we are auditing.
#
...
class AuditLogger:
    ...
    Implements the `app.domain.ports.AuditSink` Protocol (structurally — no
    explicit inheritance, to keep the domain port import-free of this adapter).
    ...
    def _enqueue(self, entry) -> None:
        ...
        some unrelated same-loop activity happens to drain it. Sync choke points
        (`git_service._run`, `event_bus.append`, CLI spawn) may run off the loop
        thread, so we hop onto the loop via `call_soon_threadsafe` whenever a
        loop is set. With no loop yet (buffering before startup), a plain
        `put_nowait` is correct because there is no waiter to wake — the worker
        drains the backlog on first `get()`.
...
# Global singleton (mirrors event_bus.event_bus). This is the process-wide
# default sink; `default_sink()` returns it so recorders resolve to it lazily.
audit_logger = AuditLogger()


def default_sink() -> AuditLogger:
    """Return the process-wide default `AuditSink`.

    Recorders call this at write time when no sink is injected, so tests that
    monkeypatch `audit_logger.record` (the singleton instance) are observed.
    """
    return audit_logger
