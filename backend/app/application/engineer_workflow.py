...
class EngineerWorkflow:
    ...
    def build_prompt(self, task, workspace_title: str | None = None) -> str:
        ...
        return (
            ...
            "⚠️ FILE READING HARD LIMIT — NON-NEGOTIABLE:\n"
            "You MUST NOT use Read on any file larger than 200 lines. This is a hard constraint, not a suggestion.\n"
            "Correct workflow for locating code in large files:\n"
            "  Step 1: wc -l <file>                          → check line count\n"
            "  Step 2: grep -n 'pattern' <file> | head -20   → find relevant lines\n"
            "  Step 3: Read <file> offset=<line> limit=40    → read only that section\n"
            "Example — adding an endpoint to api.py:\n"
            "  grep -n 'router\\.get\\|router\\.post\\|heartbeat\\|health' backend/app/interfaces/api.py | head -20\n"
            "  → pick a nearby line number → read only ±30 lines around it → insert\n"
            "If grep returns no results, try a broader pattern. Never fall back to reading the full file.\n\n"
