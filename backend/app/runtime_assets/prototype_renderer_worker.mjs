...
function layoutRules(node) {
  ...
  if (node.type === "Freeform") {
    if (item.position === void 0) rules.push("position:relative");
...
function validateNodeChildren(value, path, entityIds, minimumLength, maximumLength, positionRequired, colorTokenKeys) {
  ...
  children.forEach(
    (child, index) => validateNode(child, `${path}[${index}]`, entityIds, positionRequired, colorTokenKeys)
  );
}
function validateNode(value, path, entityIds, positionRequired, colorTokenKeys) {
  const item = record(value, path);
  const type = literal(
    item["type"],
    ["Freeform", "Stack", "Grid", "Form", "Text", "Input", "Button", "Table"],
    `${path}.type`
  );
  ...
  if (positionRequired !== hasPosition) {
    throw new RendererDocumentCodecError(
      positionRequired ? `${path}.layoutItem.position is required inside a Freeform container` : `${path}.layoutItem.position is only valid inside a Freeform container`
    );
  }
...
function parseRendererDocument(value) {
  ...
  boundedArray(item["pages"], 1, 20, "document.pages").forEach((page, index) => {
    ...
    validateNode(parsed2["root"], `document.pages[${index}].root`, entityIds, false, colorTokenKeys);
  });
...
function assertCompatibility(manifest) {
  const expected = {
    ...
    runtimeCoreBundleHash: "sha256:fc80a56f88a5319b0bd0750b8ad03ac5bd7e70ae0674a95393a2132de63c71fc",
